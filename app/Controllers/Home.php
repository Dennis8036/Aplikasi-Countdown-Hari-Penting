    ...
    public function aksi_t_acara()
{
   ...
   $model = new Gudang();
    ...
    $isi = array(
        ...
        'tanggal_acara' => $tanggal_acara,
        'status' => $status,
        'created_at' => date('Y-m-d H:i:s'),  // Set current datetime for created_at
        'updated_at' => date('Y-m-d H:i:s')   // Set current datetime for updated_at
    );
...
public function dashboard()
{
    if (session()->get('id_level') > 0) {
        ...
        $query = $model->query("
            ...
            LIMIT 1
        ");
        $data['acara_terdekat'] = $query->getRowArray();

        // Ambil acara berdasarkan tanggal dari tb_history
        $historyQuery = $model->query("
            SELECT * FROM tb_history 
            WHERE DATE(tanggal_acara) = CURDATE()
        ");
        $data['acara_history'] = $historyQuery->getResultArray();

        // Hitung total acara dari tb_acara
        $acaraQuery = $model->query("SELECT COUNT(*) AS total_acara FROM tb_acara");
        $totalAcaraResult = $acaraQuery->getRow();
        $data['total_acara'] = $totalAcaraResult->total_acara;

        // Hitung total acara selesai dari tb_history
        $totalHistoryQuery = $model->query("
            SELECT COUNT(*) AS total_history FROM tb_history 
            WHERE DATE(tanggal_acara) = CURDATE()
        ");
        $totalHistoryResult = $totalHistoryQuery->getRow();
        $data['total_history'] = $totalHistoryResult->total_history > 0 ? $totalHistoryResult->total_history : 0;
